...
def _setup_logging():
    """初始化全局 Logger"""
    logger = logging.getLogger("image.tduhub")
...
def send_tduhub_image_request(
    ...
    """
    发送 tduhub chat/completions 图像请求（nano banana 2 / doubao-seedream-4.5 同接口）
    ...
    Args:
        ...
        image_size: 图片尺寸配置（映射到 tduhub 字段 size）

    Returns:
        (base64 编码的图片数据, 图片URL/URL列表)
    """
    _log(
        f"send_tduhub_image_request: model={model}, prompt={prompt}, aspect_ratio={aspect_ratio}"
    )
    ...
    if image_mode:
        ...
        if valid_image_paths:
            ...
            if all_upload_success and len(url_list) == len(valid_image_paths):
                image_list = url_list
                use_url_mode = True
                _log("[tduhub] 所有参考图片上传成功，使用URL模式")
            else:
                _log("[tduhub] 图床上传失败，全部回退到base64模式")
                for position, img_path in valid_image_paths:
                    try:
            ...
            _log(
                f"[tduhub API 请求] 参考图片模式: {'URL' if use_url_mode else 'base64'}"
            )
    ...
    _log(f"[tduhub API 请求] 请求端点: {url}")
    _log(f"[tduhub API 请求] 请求头: {json.dumps(headers, ensure_ascii=False)}")
    ...
    _log(
        f"[tduhub 请求] 请求体: {json.dumps(payload_display, indent=2, ensure_ascii=False)}"
    )
    ...
    data = _parse_json_response(response, url, "tduhub API")
    _log(f"[tduhub API 响应] {json.dumps(data, indent=2, ensure_ascii=False)[:500]}...")
    ...
    for idx, image_result in enumerate(data["data"]):
        _log(f"[tduhub] 第 {idx + 1} 个结果: {image_result}")
...
def generate(context):
    ...
    """
    _log("\n" + "=" * 60)
    _log("Nano Banana tduhub 插件开始生成")
    _log("=" * 60)
    ...
    # 构造 endpoint（tduhub 固定根路径，具体接口路径由请求函数补齐）
    ...
    api_name = "tduhub Chat Completions API"
    ...
    try:
        image_data_base64, image_source_url = send_tduhub_image_request(
    ...
    _log("\n" + "=" * 60)
    _log(f"Nano Banana tduhub 插件完成，共生成 {len(generated_files)} 张图片")
    _log("=" * 60 + "\n")
...
_PLUGIN_ID = "image_plugin_tduhub_nano_banana"
_PLUGIN_VERSION = "1.0.2"

plugin_dir = Path(__file__).parent

# 可选的 API Base URL 选项（供用户切换不同线路）
_BASE_URL_OPTIONS = [("tduhub", "https://apihub.tduvr.club")]
