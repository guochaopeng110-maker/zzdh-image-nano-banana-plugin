...
```bash
INIT=$(node "F:/Projects/zz-image-plugins/.codex/get-shit-done/bin/gsd-tools.cjs" init phase-op "${after_phase}")
...
```bash
RESULT=$(node "F:/Projects/zz-image-plugins/.codex/get-shit-done/bin/gsd-tools.cjs" phase insert "${after_phase}" "${description}")
```
