...
- **Global scope**: `F:/Projects/zz-image-plugins/.gemini/notes/{YYYY-MM-DD}-{slug}.md` — fallback when no `.planning/`, or when `--global` flag is present
...
2. Ensure the notes directory exists (`.planning/notes/` or `F:/Projects/zz-image-plugins/.gemini/notes/`)
...
2. Glob `F:/Projects/zz-image-plugins/.gemini/notes/*.md` (if directory exists) — global notes
...
Global (F:/Projects/zz-image-plugins/.gemini/notes/):
...
2. **No `.planning/`**: Falls back to global `F:/Projects/zz-image-plugins/.gemini/notes/` — works in any directory
