...
```bash
node "F:/Projects/zz-image-plugins/.gemini/get-shit-done/bin/gsd-tools.cjs" audit-open 2>/dev/null
```
...
for summary in .planning/phases/*-*/*-SUMMARY.md; do
  [ -e "$summary" ] || continue
  node "F:/Projects/zz-image-plugins/.gemini/get-shit-done/bin/gsd-tools.cjs" summary-extract "$summary" --fields one_liner --pick one_liner
done
```
...
```bash
ARCHIVE=$(node "F:/Projects/zz-image-plugins/.gemini/get-shit-done/bin/gsd-tools.cjs" milestone complete "v[X.Y]" --name "[Milestone Name]")
```
...
**If doesn't exist:** Create from template at `F:/Projects/zz-image-plugins/.gemini/get-shit-done/templates/retrospective.md`.
...
**Commit:**
```bash
node "F:/Projects/zz-image-plugins/.gemini/get-shit-done/bin/gsd-tools.cjs" commit "docs: update retrospective for v${VERSION}" --files .planning/RETROSPECTIVE.md
```
...
```bash
INIT=$(node "F:/Projects/zz-image-plugins/.gemini/get-shit-done/bin/gsd-tools.cjs" init execute-phase "1")
...
Detect base branch:
```bash
BASE_BRANCH=$(node "F:/Projects/zz-image-plugins/.gemini/get-shit-done/bin/gsd-tools.cjs" config-get git.base_branch 2>/dev/null || echo "")
