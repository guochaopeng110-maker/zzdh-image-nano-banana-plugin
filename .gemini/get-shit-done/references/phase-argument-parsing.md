...
```bash
PHASE_INFO=$(node "F:/Projects/zz-image-plugins/.gemini/get-shit-done/bin/gsd-tools.cjs" find-phase "${PHASE}")
```
...
```bash
PHASE_CHECK=$(node "F:/Projects/zz-image-plugins/.gemini/get-shit-done/bin/gsd-tools.cjs" roadmap get-phase "${PHASE}" --pick found)
...
```bash
PHASE_DIR=$(node "F:/Projects/zz-image-plugins/.gemini/get-shit-done/bin/gsd-tools.cjs" find-phase "${PHASE}" --raw)
```
