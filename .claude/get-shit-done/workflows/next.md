...
```bash
# Get state snapshot
node "F:/Projects/zz-image-plugins/.claude/get-shit-done/bin/gsd-tools.cjs" state json 2>/dev/null || echo "{}"
```
...
```bash
node "F:/Projects/zz-image-plugins/.claude/get-shit-done/bin/gsd-tools.cjs" commit "docs: defer incomplete Phase {src} items to backlog"
```
